...
class SearchWordController extends Controller
{
	...
	{
		\Log::debug("searchEn in SearchWordController");
		$word = $request->word;
		...
		preg_match('/<ul class="list_search" >(.*?)<\/ul>/is', $result, $means);
		$mean = $means[1];
		preg_match('/<li><span class=(.*?)<\/span><\/li>/is', $result, $means);
		\Log::debug($mean);
