...
class MemberController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        \Log::debug("MemberController in login");
        $validator = Validator::make($request->all(), [
    ...
    {
        \Log::debug('MemberC in check func');
        \Log::debug($request);
        if (!$request->token) {
            return json([
        ...
        return json_encode(['member_pk' => 1, "email" => "123@gmail.com", "nickname" => "YJU"]);
    }
    ...
    {
        \Log::debug("myVideo in MemerController");
        $member_pk = 1;
